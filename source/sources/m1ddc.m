...
static DDCValue readingOperation(DDCTransport *transport, DDCPacket *packet) {
    ...
    if (getenv("LG_DDC_DEBUG")) {
        fprintf(stderr, "DDC reply:");
        for (int i=0; i<12; i++) fprintf(stderr, " %02x", readPacket.data[i]);
        fprintf(stderr, "\n");
    }
    UInt8 checksum = 0x50;
    for (int i=0; i<11; i++) checksum ^= readPacket.data[i];
    if (readPacket.data[0] != 0x6e || readPacket.data[1] != 0x88 ||
        readPacket.data[2] != 0x02 || readPacket.data[3] != 0 ||
        readPacket.data[4] != packet->data[2] || checksum != 0) {
        fprintf(stderr, "No valid matching VCP response\n");
        return dummyAttr;
    }
...
int main(int argc, char** argv) {
    ...
    if (STR_EQ(argv[0], "probe") && CGDisplayIsAsleep(selectedDisplay->id)) {
        writeToStdOut(@"{\"power\":-1,\"online\":true,\"asleep\":true}\n");
        return EXIT_SUCCESS;
    }
    ...
    if (!STR_EQ(argv[0], "set")) {
        ...
        if (displayAttr.curValue == -1 && !STR_EQ(argv[0], "probe")) {
            return EXIT_FAILURE;
        }
    }

    if (STR_EQ(argv[0], "probe")) {
        DisplayInfos connected[MAX_DISPLAYS];
        int externalCount = getOnlineDisplayInfos(connected);
        writeToStdOut([NSString stringWithFormat:@"{\"power\":%i,\"online\":%s,\"asleep\":%s,\"display_id\":%u,\"external_count\":%i}\n",
          displayAttr.curValue, CGDisplayIsOnline(selectedDisplay->id) ? "true" : "false",
          CGDisplayIsAsleep(selectedDisplay->id) ? "true" : "false", selectedDisplay->id, externalCount]);
        return EXIT_SUCCESS;
    }
